function [TrainTable,blds,ds] = generateTablesSSD(train_ann_files,imds)
    ...
    NL=cell(1,NoTrAnn);
    cnt = 0;
    cd 'C:\Users\yalci\Desktop\ImplementatioN\Fm\train\';
    disp("OK!");
    pause(10);
    for j=1:NoTrAnn
    ...
    cd C:\Users\yalci\Desktop\ImplementatioN\Fm;
    pause(2);
    disp(cnt);
    ...
    for i=1:r
       ...
       if isempty(TrainTable.PO1{i})
           TrainTable.PO1{i} = [1 1 2 2];
       end
       if isempty(TrainTable.PO2{i})
           TrainTable.PO2{i} = [1 1 2 2];
       end
       if isempty(TrainTable.CO{i})
           TrainTable.CO{i} = [1 1 2 2];
       end
       if isempty(TrainTable.MP{i})
           TrainTable.MP{i} = [1 1 2 2];
       end
       if isempty(TrainTable.TA{i})
           TrainTable.TA{i} = [1 1 2 2];
       end
       if isempty(TrainTable.WA{i})
           TrainTable.WA{i} = [1 1 2 2];
       end
       if isempty(TrainTable.LA{i})
           TrainTable.LA{i} = [1 1 2 2];
       end
       if isempty(TrainTable.NL{i})
           TrainTable.NL{i} = [1 1 2 2];
       end
    end
